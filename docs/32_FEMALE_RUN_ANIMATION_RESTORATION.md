...
作成日: 2026-09-22（JST）。対象は女性 `runner_002` のWeb人物。Sites version 20で配信中のv5と、GitHub mainにある未公開の動画再生修正を起点とする。
...
4. Web人物定義をv6へ切り替える。新SHAの外観・一般公開同意は未取得のため、manifestは`draft`とし、Sites sourceへのpush、Sites保存・公開をしない。GitHubはprivate repositoryに反映する。
...
| FRA-07 | v6 exact SHAの外観承認と一般公開・配信・cache／取得・ゲーム表現への同意 | BLOCKED |
| FRA-08 | Sites source・保存version・公開deployment | NOT STARTED（今回依頼の対象外） |
