...
import { challengeDateInTokyo, CONTINUOUS_RULESET_ID, DIFFICULTY_RULESET_IDS, FIXED_DIFFICULTY_RULESET_IDS, FRONT_RULESET_ID, LEGACY_DIFFICULTY_RULESET_IDS, LEGACY_RULESET_ID, PREVIOUS_DIFFICULTY_RULESET_IDS, RULESET_ID } from "@/lib/game-core";

const allowedRulesets = new Set<string>([
  ...Object.values(DIFFICULTY_RULESET_IDS),
  ...Object.values(FIXED_DIFFICULTY_RULESET_IDS),
  ...Object.values(PREVIOUS_DIFFICULTY_RULESET_IDS),
...
export async function GET(request: Request) {
  ...
  const rows = await getD1()
    .prepare(
      "SELECT p.display_name, b.distance_cm, b.duration_ticks, b.achieved_at_ms, r.terminal_reason FROM best_scores b JOIN players p ON p.player_id = b.player_id JOIN runs r ON r.run_id = b.best_run_id WHERE b.challenge_date = ? AND b.ruleset_id = ? AND r.status = 'VERIFIED' ORDER BY b.distance_cm DESC, b.duration_ticks ASC, b.achieved_at_ms ASC, b.best_run_id ASC LIMIT 100",
    )
    .bind(challengeDate, rulesetId)
